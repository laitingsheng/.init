...
def _default_install_base() -> Path:
    if os.geteuid() == 0:
        return Path("/usr/local/share/prek")
    xdg_data = os.environ.get("XDG_DATA_HOME")
    if xdg_data:
        return Path(xdg_data) / "prek"
    return Path.home() / ".local" / "share" / "prek"
...
def main() -> None:
    ...
    if not force and binary_path.is_file() and link.is_symlink() and link.resolve() == binary_path.resolve():
        module.exit_json(changed=False, msg=f"prek {version} already installed")

    if module.check_mode:
        module.exit_json(changed=True, msg=f"Would install prek {version}")

    target_triple = f"{arch}-{OS_TRIPLE[os_name]}"
    url = f"https://github.com/j178/prek/releases/download/v{version}/prek-{target_triple}.tar.gz"
    ...
    member_name = f"prek-{target_triple}/prek"
    ...
    module.exit_json(changed=True, msg=f"Installed prek {version}")
